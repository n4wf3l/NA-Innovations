        ...
        <div className="max-w-3xl mx-auto">
            <form onSubmit={onSubmit} className="space-y-6">
                ...
                <details className="group">
                    ...
                    <div className="space-y-4 pt-2 pb-1 animate-tab-in">
                        {/* Existing website */}
                        <div>
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={!!(quoteForm as any).has_website}
                                    onChange={e => setQuoteForm({ ...quoteForm, has_website: e.target.checked, website_url: e.target.checked ? (quoteForm as any).website_url || '' : '' } as any)}
                                    className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-teal-500 focus:ring-teal-500"
                                />
                                <span className="text-sm text-gray-700 dark:text-gray-300">{t('I already have a website')}</span>
                            </label>
                            {(quoteForm as any).has_website && (
                                <input
                                    type="url"
                                    value={(quoteForm as any).website_url || ''}
                                    onChange={e => setQuoteForm({ ...quoteForm, website_url: e.target.value } as any)}
                        ...
                        {/* How did you find us */}
                        <div>
                            <label className={labelClasses}>{t('How did you find us?')}</label>
                            <select
                                value={(quoteForm as any).source || ''}
                                onChange={e => setQuoteForm({ ...quoteForm, source: e.target.value } as any)}
                                className={inputClasses}
                            >
                                <option value="">{t('Select...')}</option>
                                <option value="google">{t('Google search')}</option>
                                <option value="social_media">{t('Social media')}</option>
                                <option value="referral">{t('Word of mouth')}</option>
                                <option value="partner">{t('Partner / Referral')}</option>
                                <option value="portfolio">{t('Saw your portfolio')}</option>
                        ...
                        {/* Preferred language */}
                        <div>
                            <label className={labelClasses}>{t('Preferred language for communication')}</label>
                            <div className="flex gap-3">
                                {[
                                    ...
                                    <label key={lang.code} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border cursor-pointer transition-all text-sm font-medium ${
                                        (quoteForm as any).preferred_lang === lang.code
                                            ? 'border-teal-400 bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-300'
                                        ...
                                        <input
                                            ...
                                            name="preferred_lang"
                                            value={lang.code}
                                            checked={(quoteForm as any).preferred_lang === lang.code}
                                            onChange={e => setQuoteForm({ ...quoteForm, preferred_lang: e.target.value } as any)}
                                            className="sr-only"
                                        />
                                        {lang.label}
                                    </label>
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-400 mb-3">{t('Add any relevant documents: brief, mockups, specifications... (PDF, DOC, images -- max 10MB each)')}</p>
                    <label className="flex items-center justify-center w-full h-24 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-teal-400 hover:bg-teal-50/50 dark:hover:bg-teal-500/10 transition-all group">
