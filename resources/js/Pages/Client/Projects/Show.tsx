...
export default function ClientProjectShow({ project, quotes, invoices, services, notes, projectDocuments, attachments = [], techDocs = [], hasTestimonial = false }: Props) {
    ...
    // Pagination — slice the unified timeline to TIMELINE_PAGE_SIZE items.
    const totalPages = Math.max(1, Math.ceil(unifiedTimeline.length / TIMELINE_PAGE_SIZE));
    const safePage = Math.min(timelinePage, totalPages - 1);
    const paginatedTimeline = unifiedTimeline.slice(
        safePage * TIMELINE_PAGE_SIZE,
        safePage * TIMELINE_PAGE_SIZE + TIMELINE_PAGE_SIZE,
    );
                            ...
                            <div className="p-6">
                                {unifiedTimeline.length === 0 ? (
                                    ...
                                    <div className="relative">
                                        ...
                                        {/* Pagination controls — visible only when more than one page */}
                                        {totalPages > 1 && (
                                            <div className="mt-6 flex items-center justify-between border-t border-gray-100 dark:border-gray-700 pt-4">
                                                <button
                                                    ...
                                                    disabled={safePage === 0}
                                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                                                >
                                                ...
                                                <span className="text-xs text-gray-500 dark:text-gray-400">
                                                    {t('Page :page of :total', { page: safePage + 1, total: totalPages } as any)
