...
export default function InvoiceCreate({ clients }: Props) {
    ...
    return (
        <AdminLayout title={t("New Invoice")} header={t("New Invoice")}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); router.post('/admin/invoices', { ...data, items } as any); }} className="space-y-6 max-w-5xl">
                {/* Client Selection */}
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value={data.title} onChange={e => setData('title', e.target.value)} className={inputClass} required placeholder="e.g. Website Development - Final Invoice" />
                                ...
                                {items.map((item, index) => (
                                    <tr key={index} className="border-b border-gray-50">
                                        <td className="py-2 px-2">
                                            <input type="text" value={item.description} onChange={e => updateItem(index, 'description', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-teal-400 focus:ring-teal-400" placeholder={t("Item description")} required />
                                        </td>
                                        <td className="py-2 px-2">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea value={data.notes} onChange={e => setData('notes', e.target.value)} rows={2} className={inputClass} placeholder={t("Internal notes (not shown on invoice)...")} />
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex items-center justify-end gap-3">
                    <Link href="/admin/invoices" className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{t('Cancel')}</Link>
