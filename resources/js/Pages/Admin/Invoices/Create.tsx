...
export default function InvoiceCreate({ clients }: Props) {
    ...
    const handleExternalSubmit = (e: React.FormEvent) => {
        ...
        formData.append('due_date', externalForm.due_date);
        formData.append('notes', externalForm.notes);
        formData.append('is_signed', externalIsSigned ? '1' : '0');
        router.post('/admin/invoices/upload-external', formData, {
            forceFormData: true,
    ...
    const validateStep1 = (): boolean => {
        ...
        if (!form.issue_date) errors.issue_date = t('Ce champ est requis');
        if (!form.due_date) {
            errors.due_date = t('Ce champ est requis');
        } else if (form.issue_date && form.due_date < form.issue_date) {
            errors.due_date = t("La date d'échéance doit être supérieure ou égale à la date d'émission");
        }
    ...
    const stepHasErrors = (stepNum: number): boolean => {
        ...
        if (stepNum === 1) return keys.some(k => ['client_name', 'client_email', 'title', 'issue_date', 'due_date', 'type', 'tax_rate'].includes(k));
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (!form.issue_date) step1Errors.issue_date = t('Ce champ est requis');
        if (!form.due_date) {
            step1Errors.due_date = t('Ce champ est requis');
        } else if (form.issue_date && form.due_date < form.issue_date) {
            step1Errors.due_date = t("La date d'échéance doit être supérieure ou égale à la date d'émission");
        }
        ...
        setSubmitting(true);
        router.post('/admin/invoices', { ...form, items, locale: form.locale } as any, {
            onFinish: () => setSubmitting(false),
            onError: (errors) => {
                const keys = Object.keys(errors);
                if (keys.some(k => ['client_name', 'client_email', 'title', 'issue_date', 'due_date', 'type', 'tax_rate'].includes(k))) setStep(1);
    ...
    return (
        <AdminLayout title={t("New Invoice")} header={t("New Invoice")}>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                    <div className="flex items-center gap-4">
                        <button type="button" onClick={() => setMode('platform')}
                            className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${mode === 'platform' ? 'border-blue-400 bg-blue-50 dark:bg-blue-500/10' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${mode === 'platform' ? 'bg-blue-400 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                            ...
                            <div className="text-left">
                                <p className={`text-sm font-bold ${mode === 'platform' ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'}`}>{t('Create on platform')}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t('Use our invoice builder with automatic calculations')}</p>
                            </div>
                        </button>
                        <button type="button" onClick={() => setMode('external')}
                            className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${mode === 'external' ? 'border-blue-400 bg-blue-50 dark:bg-blue-500/10' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${mode === 'external' ? 'bg-blue-400 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                            ...
                            <div className="text-left">
                                <p className={`text-sm font-bold ${mode === 'external' ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'}`}>{t('Upload my own document')}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t('Upload an existing invoice as PDF')}</p>
                            </div>
                        </button>
                    </div>
                </div>
                ...
                <form onSubmit={handleExternalSubmit} className="max-w-4xl mx-auto space-y-6">
                    <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">{t('The uploaded invoice must be signed by both parties to be valid. Only invoices with confirmed payment can be marked as paid.')}</p>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <SearchableSelect value={externalForm.client_id} onChange={handleExternalClientSelect} placeholder={t("-- Manual entry --")} searchPlaceholder={t("Search clients...")}
                                    options={[{ value: '', label: t("-- Manual entry --") }, ...clients.map(c => ({ value: String(c.id), label: c.name, sublabel: c.company_name || undefined, icon: <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center flex-shrink-0"><span className="text-[10px] font-bold text-white">{c.name.split(' ').map((w: string) => w[0]).join('').slice(0,2)}</span></div> }))]} />
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div><label className={label}>{t('Due date')}</label><input type="date" value={externalForm.due_date} onChange={e => setExt('due_date', e.target.value)} className={input} /></div>
                        </div>
                        <div><label className={label}>{t('Notes')}</label><textarea value={externalForm.notes} onChange={e => setExt('notes', e.target.value)} rows={3} className={input} placeholder={t('Internal notes (not shown on invoice)...')} /></div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{t('Document file')}</h3>
                        <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer ${validationErrors.ext_file ? 'border-red-300 bg-red-50 dark:bg-red-500/5' : 'border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 bg-gray-50 dark:bg-gray-700/30'}`}
                    ...
                    <div className="flex justify-end gap-3">
                        <Link href="/admin/invoices" className="px-6 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{t('Cancel')}</Link>
                        <button type="submit" disabled={submitting} className="px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-500/25 transition-all disabled:opacity-50 flex items-center gap-2">
                            {submitting ? (<><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{t('Uploading...')}</>) : (<><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /></svg>{t('Upload invoice')}</>)}
                        </button>
                    </div>
                </form>
            )}
            ...
            <form onSubmit={submit} className="max-w-4xl mx-auto">
                {step === 1 && (
                    <Step1ClientDetails form={form} set={set} handleClientSelect={handleClientSelect} clients={clients}
                        input={input} label={label} errorRing={errorRing} FieldError={FieldError} validateStep1={validateStep1}
                        setStep={setStep} ErrorBanner={ErrorBanner} invoiceTypes={invoiceTypes} />
                )}
                {step === 2 && (
                ...
                {step === 3 && (
                    <Step3Settings form={form} set={set} subtotal={subtotal} tax={tax} total={total}
                        items={items} submitting={submitting} input={input} label={label} goToStep={goToStep} ErrorBanner={ErrorBanner} />
                )}
            </form>
            </>)}
        </AdminLayout>
