...
export default function ServiceEdit({ service, clients, projects }: Props) {
    ...
    return (
        <AdminLayout title={t("Edit Service")} header={t("Edit Recurring Service")}>
            ...
            <div className="mb-6">
                <Link href={`/admin/services/${service.id}`} className="text-sm text-gray-500 hover:text-gray-700">&larr; {t("Back to Service")}</Link>
            </div>

            <form onSubmit={submit} className="space-y-6 max-w-4xl">
                {/* Service Info */}
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">{t("Service Information")}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Provider Account")}</label>
                            <input type="text" value={data.provider_account} onChange={e => setData('provider_account', e.target.value)} className={inputClass} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Provider Reference")}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Login URL")}</label>
                ...
                {/* Assignment */}
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">{t("Assignment")}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                ...
                {/* Schedule & Pricing */}
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">{t("Schedule & Pricing")}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Expiry Date")} *</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Real Cost")} *</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Billed Price")} *</label>
                        ...
                        <div>
                            ...
                            <div className={`${inputClass} bg-gray-50 ${margin >= 0 ? 'text-emerald-600' : 'text-red-600'} font-medium`}>
                                {margin.toFixed(2)}
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Payment Mode")}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Alert Days Before")}</label>
                        ...
                        <div className="flex items-end">
                            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 pb-3">
                ...
                {/* Credentials & Notes */}
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">{t("Credentials & Notes")}</h3>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("Credentials Note")}</label>
                ...
                <div className="flex items-center justify-end gap-3">
                    <Link href={`/admin/services/${service.id}`} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{t('Cancel')}</Link>
