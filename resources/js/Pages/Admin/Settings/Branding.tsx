            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
                    ...
                    <div className="mb-6">
                        ...
                        {/* Logo actuel (DB ou fallback) */}
                        <div className="flex items-start gap-6 p-4 rounded-xl bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 mb-4">
                            <div className="flex gap-3">
                                <div className="flex flex-col items-center gap-1.5">
                                    <div className="w-20 h-20 rounded-xl bg-gray-900 dark:bg-gray-950 flex items-center justify-center p-3">
                                        <img
                                            src={branding.logo_path ? `/storage/${branding.logo_path}` : '/white-logo-small.png'}
                                            alt={t('Logo fond sombre')}
                                            className="max-w-full max-h-full object-contain"
                                        />
                                    </div>
                                    <span className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t('Sombre')}</span>
                                </div>
                                <div className="flex flex-col items-center gap-1.5">
                                    <div className="w-20 h-20 rounded-xl bg-white dark:bg-gray-100 border border-gray-200 flex items-center justify-center p-3">
                                        <img
                                            src={branding.logo_path ? `/storage/${branding.logo_path}` : '/dark-logo-small.png'}
                                            alt={t('Logo fond clair')}
                                            className="max-w-full max-h-full object-contain"
                                        />
                                    </div>
                                    <span className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t('Clair')}</span>
                                </div>
                            </div>
                            <div className="flex-1 pt-1">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${branding.logo_path ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300'}`}>
                                        {branding.logo_path ? (
                                            <><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg> {t('Logo personnalisé')}</>
                                        ) : (
                                            <><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg> {t('Logo par défaut')}</>
                                        )}
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                                    {branding.logo_path
                                        ? t("Ce logo personnalisé est utilisé partout sur la plateforme (sidebar, landing page, emails). Vous pouvez le remplacer ou le supprimer pour revenir au logo par défaut.")
                                        : t("Le logo par défaut (fichiers white-logo-small.png et dark-logo-small.png) est utilisé partout. Uploadez un logo personnalisé pour le remplacer.")
                                    }
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors text-sm font-medium">
                                ...
                                {uploadingLogo ? t('Envoi en cours...') : branding.logo_path ? t('Remplacer le logo') : t('Télécharger un logo')}
                                <input
                                    type="file"
                            ...
                            {branding.logo_path && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                                >
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                    {t('Revenir au logo par défaut')}
                                </button>
                            )}
                            <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, SVG, WebP. Max 2 Mo.</p>
                        </div>
                    </div>
