...
export default function ProjectEdit({ project, clients, developers, leads }: Props) {
    ...
    return (
        <AdminLayout title={t("Edit Project")} header={t("Edit Project")}>
            ...
            <div className="mb-6">
                <Link href={`/admin/projects/${project.id}`} className="text-sm text-gray-500 hover:text-gray-700">&larr; {t("Back to Project")}</Link>
            </div>
            ...
            <form onSubmit={submit} className="space-y-6 max-w-4xl">
                ...
                <div className="flex items-center justify-end gap-3">
                    <Link href={`/admin/projects/${project.id}`} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{t('Cancel')}</Link>
