...
interface SimulationSummary {
    id: number; name: string; product_name: string | null; monthly_price: string;
    time_horizon: number; team_member_count: number; updated_at: string;
    total_projected_revenue: number; total_profit: number; break_even_month: number | null;
}
...
export default function SimulatorTab({ simulations }: Props) {
        ...
        <div>
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-700 p-6 sm:p-8 mb-6">
                ...
                <div className="relative flex items-start justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-white mb-1">{t('Simulateur financier')}</h2>
                        <p className="text-indigo-200 text-sm">{t('Évaluez la rentabilité de vos projets SaaS avant de les lancer.')}</p>
                    </div>
                    <Link href="/admin/simulator/create" className="flex items-center gap-2 px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white text-sm font-semibold rounded-xl transition-colors shrink-0">
            ...
            {simulations.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-16 text-center">
                    <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center mx-auto mb-4">
                        <svg className="w-8 h-8 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75V18m-7.5-6.75h.008v.008H8.25v-.008zm0 2.25h.008v.008H8.25V13.5zM9.75 6.75h4.5a2.25 2.25 0 012.25 2.25v.75a2.25 2.25 0 01-2.25 2.25h-4.5a2.25 2.25 0 01-2.25-2.25V9a2.25 2.25 0 012.25-2.25z" /></svg>
                    </div>
                    <p className="text-gray-500 dark:text-gray-400 mb-1">{t('Aucune simulation')}</p>
                    <p className="text-sm text-gray-400 dark:text-gray-500 mb-6">{t('Créez votre première simulation financière.')}</p>
                        ...
                        <div key={sim.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-700 transition-all duration-200 group">
                            <Link href={`/admin/simulator/${sim.id}`} className="block p-6">
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-4">
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-2.5 text-center">
                                        <p className="text-xs text-gray-400 dark:text-gray-500">Break-even</p>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white">{sim.break_even_month ? `M${sim.break_even_month}` : '-'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex border-t border-gray-50 dark:border-gray-700">
                                <button onClick={() => router.post(`/admin/simulator/${sim.id}/duplicate`)} className="flex-1 py-2.5 text-xs font-medium text-gray-400 hover:text-indigo-500 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">{t('Dupliquer')}</button>
                                <div className="w-px bg-gray-50 dark:bg-gray-700" />
                                <button onClick={() => { if (confirm(t('Supprimer cette simulation ?'))) router.delete(`/admin/simulator/${sim.id}`); }} className="flex-1 py-2.5 text-xs font-medium text-gray-400 hover:text-red-500 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">{t('Supprimer')}</button>
                            </div>
                        </div>
                    ))}
                </div>
