        ...
        <>
            {financialUnlocked ? (
                ...
                <div className="flex items-center space-x-2">
                    {/* Timer badge */}
                    <div className="relative flex items-center bg-emerald-50 dark:bg-emerald-500/10 rounded-lg overflow-hidden" title={`Auto-lock in ${formatTime(remaining)}`}>
                        {/* Progress bar background */}
                        <div
                            className="absolute inset-0 bg-emerald-100 dark:bg-emerald-500/20 transition-all duration-1000 ease-linear"
                            style={{ width: `${progressPct}%` }}
                        />
                        <div className="relative flex items-center space-x-1.5 px-3 py-1.5">
                            <svg className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
                            <span className={`text-xs font-bold tabular-nums ${remaining < 120 ? 'text-red-600 dark:text-red-400' : 'text-emerald-700 dark:text-emerald-300'}`}>
                ...
                <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all"
            ...
            {showModal && createPortal(
                <div className="fixed inset-0 z-[99999] flex items-center justify-center p-6">
                    ...
                    <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-xs p-6 animate-modal text-center">
                        <div className="w-14 h-14 bg-gray-900 dark:bg-gray-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
                        ...
                        <h3 className="text-lg font-black text-gray-900 dark:text-white mb-1">{t('Enter PIN')}</h3>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">{t('Visible for 15 minutes after unlock')}</p>

                        {error && <p className="text-sm text-red-500 bg-red-50 dark:bg-red-500/10 rounded-xl px-3 py-2 mb-4">{error}</p>}
                        ...
                        <input
                            ...
                            placeholder="••••••"
                            className="w-full text-center text-2xl font-mono tracking-[0.5em] border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700/50 rounded-xl py-4 text-gray-900 dark:text-white focus:border-gray-900 dark:focus:border-teal-400 focus:ring-gray-900 dark:focus:ring-teal-400 placeholder-gray-200 dark:placeholder-gray-600"
                        />

                        <button
                            onClick={handleUnlock}
                            disabled={pin.length < 4 || loading}
                            className="w-full mt-4 py-3.5 bg-gray-900 dark:bg-teal-600 text-white text-sm font-bold rounded-xl disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center"
                        >
                            {loading ? (
                        ...
                        <button onClick={() => setShowModal(false)} className="mt-3 text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">{t('Cancel')}</button>
                    </div>
                </div>,
                document.body
            )}
