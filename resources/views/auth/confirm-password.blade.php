...
<head>
    ...
    <title>Confirm Password - {{ $brandCompanyName }}</title>
...
<body class="font-body antialiased bg-animated min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="absolute top-1/2 right-10 w-36 h-36 bg-amber-300/5 rounded-full blur-3xl" style="animation: float 6s ease-in-out infinite 3s;"></div>

    <div class="w-full max-w-md relative z-10" x-data="{ submitting: false, showPassword: false }">
        ...
        <div class="bg-slate-800/60 backdrop-blur-2xl border border-slate-700/40 rounded-2xl shadow-2xl shadow-black/20 overflow-hidden">
            ...
            <div class="p-8 sm:p-10">
                ...
                {{-- Header --}}
                <h2 class="font-display text-3xl text-white tracking-wide mb-2 text-center fade-in-d1">{{ __('Confirm Password') }}</h2>
                ...
                <form method="POST" action="{{ route('password.confirm') }}" class="space-y-5 fade-in-d3" @submit="submitting = true">
                    @csrf
