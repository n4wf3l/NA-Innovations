...
@section('title', __('Edit Quote') . ' - ' . $quote->quote_number)
...
@section('content')
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit Quote') }} {{ $quote->quote_number }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ __('Modify this quote. Changes are saved as draft until you send it.') }}</p>
    </div>

    <!-- Form -->
    <form method="POST" action="{{ route('admin.quotes.update', $quote) }}" x-data="quoteForm()">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <select name="lead_id"
                        ...
                        @foreach($leads as $lead)
                            <option value="{{ $lead->id }}" {{ old('lead_id', $quote->lead_id) == $lead->id ? 'selected' : '' }}>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $quote->title) }}"
                           class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300" required>
                ...
                <div>
                    ...
                    <textarea name="introduction" id="introduction" rows="3"
                              class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300">{{ old('introduction', $quote->introduction) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="scope_of_work" id="scope_of_work" rows="4"
                              class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300">{{ old('scope_of_work', $quote->scope_of_work) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="exclusions" id="exclusions" rows="2"
                              class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300">{{ old('exclusions', $quote->exclusions) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="mt-5">
                ...
                <textarea name="terms_and_conditions" id="terms_and_conditions" rows="3"
                          class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300">{{ old('terms_and_conditions', $quote->terms_and_conditions) }}</textarea>
            </div>

            <div class="mt-5">
                <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Notes') }}</label>
                <textarea name="notes" id="notes" rows="2"
                          class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-teal-300 focus:ring-teal-300">{{ old('notes', $quote->notes) }}</textarea>
            </div>
        </div>
    ...
    <script>
        function quoteForm() {
            return {
                clientName: @json(old('client_name', $quote->client_name)),
                clientEmail: @json(old('client_email', $quote->client_email)),
                clientCompany: @json(old('client_company', $quote->client_company ?? '')),
                clientAddress: @json(old('client_address', $quote->client_address ?? '')),
                clientVat: @json(old('client_vat', $quote->client_vat ?? '')),
                taxRate: {{ old('tax_rate', $quote->tax_rate) }},
                depositPercentage: {{ old('deposit_percentage', $quote->deposit_percentage) }},
                items: @json($quote->items->map(fn($item) => [
