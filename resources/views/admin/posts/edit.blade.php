...
@section('content')
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit Post') }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ __('Edit this article.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data">
        ...
        <x-admin.card>
            <div class="p-6 space-y-5">
                <x-admin.input name="title" label="{{ __('Title') }}" :required="true" :value="$post->title" placeholder="{{ __('Post title') }}" />

                <x-admin.input name="subject" label="{{ __('Subject') }}" :required="true" :value="$post->subject" placeholder="{{ __('Post subject') }}" />

                <x-admin.textarea name="description" label="{{ __('Description') }}" :required="true" :rows="6" :value="$post->description" placeholder="{{ __('Write your post content here...') }}" />
                ...
                <div x-data="{ preview: null }">
                    ...
                    @if($post->photo)
                        <div class="mt-2 mb-3">
                            <p class="text-xs text-gray-500 mb-1">{{ __('Current photo:') }}</p>
                            <img src="{{ Storage::url($post->photo) }}" alt="{{ $post->title }}" class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                        </div>
                    @endif
                    ...
                    <template x-if="preview">
                        <div class="mt-3">
                            <p class="text-xs text-gray-500 mb-1">{{ __('New photo preview:') }}</p>
