...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Referral Partners') }}</h1>
    ...
    <p class="text-sm text-gray-500 -mt-4 mb-6">{{ __('Manage your referral partners. Each partner gets a unique referral link to track the leads they bring in.') }}</p>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($partners as $partner)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <code class="px-2 py-1 bg-gray-100 rounded text-xs font-mono text-gray-700">{{ $partner->referral_code }}</code>
                                </td>
                                <td class="px-4 py-3 text-gray-600">
    ...
    @else
        ...
        <x-admin.card>
            <x-admin.empty-state
                title="{{ __('No partners yet') }}"
                description="{{ __('No referral partners found. Add your first partner to get started.') }}"
                :actionUrl="route('admin.partners.create')"
                actionLabel="{{ __('Add Partner') }}"
            />
        </x-admin.card>
    @endif
@endsection
